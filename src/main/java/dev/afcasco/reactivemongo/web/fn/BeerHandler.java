...
import dev.afcasco.reactivemongo.service.BeerService;
...
public class BeerHandler {
    ...
    public Mono<ServerResponse> listBeers(ServerRequest serverRequest) {
        ...
        if (serverRequest.queryParam("beerStyle").isPresent()) {
            flux = beerService.findAllByBeerStyle(serverRequest.queryParam("beerStyle").get());
        } else {
    ...
    public Mono<ServerResponse> createBeer(ServerRequest serverRequest) {
        return beerService.saveBeer(serverRequest.bodyToMono(BeerDTO.class)
                ...
                .flatMap(beerDTO -> ServerResponse.created(UriComponentsBuilder
                                .fromPath(BeerRouterConfig.BEER_PATH_ID).build(beerDTO.getId()))
                        .build());
    }
